...
fn benchmark_mask_improver_v3a_features() {
    ...
    println!("  ✓ Has 'When Creating New Masks' section: {}", has_creation_workflow);
    ...
    println!("  ✓ Has Benchmark Analysis framework: {}", has_benchmark_analysis);
    println!("  ✓ Has v3A identity markers: {}", has_v3a_identity);
    println!("  ✓ Has v3A improvement notes: {}", has_v3a_notes);
    println!();

    // Assertions
    assert!(has_identity && has_expertise && has_mission, "v2 structure missing");
    assert!(has_examples && has_validation && has_prioritization, "v2 features missing");
    ...
    assert!(has_benchmark_analysis, "v3A: Benchmark analysis framework missing!");

    let v2_score = [has_identity, has_expertise, has_mission, has_examples, has_validation, has_prioritization]
        .iter()
        .filter(|&&x| x)
        .count();

    let v3a_score = [has_creation_workflow, has_7_steps, has_pattern_library, has_patterns,
                     has_benchmark_analysis, has_v3a_identity, has_v3a_notes]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    if v2_score == 6 && v3a_score == 7 {
        ...
        panic!("v3A features incomplete: v2={}/6, v3A={}/7", v2_score, v3a_score);
    }
}
...
fn benchmark_distributed_systems_structure() {
    ...
    let has_consensus = content.contains("Consensus Algorithms") || content.contains("Paxos") || content.contains("Raft");
    ...
    // Quality markers
    let has_real_systems = content.contains("PostgreSQL") || content.contains("etcd") || content.contains("HAProxy");
    let has_tradeoffs = content.contains("Trade-offs:") || content.contains("Trade-off");
    let has_configs = content.contains("postgresql.conf") || content.contains("haproxy.cfg") || content.contains("Configuration:");
    ...
    // Assertions
    assert!(has_identity && has_expertise && has_mission, "Basic structure missing");
    assert!(has_guidelines && has_examples && has_notes, "Required sections missing");
    assert!(has_consensus && has_cap && has_ha_patterns, "Core domain expertise missing");
    assert!(has_real_systems, "Should reference real systems, not abstract examples");
    assert!(has_tradeoffs, "Should include trade-off analysis");

    let structure_score = [has_identity, has_expertise, has_mission, has_guidelines, has_examples, has_notes]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    if structure_score == 6 && domain_score == 3 && quality_score >= 2 {
        ...
        panic!("Mask quality insufficient: structure={}/6, domain={}/3, quality={}/3",
