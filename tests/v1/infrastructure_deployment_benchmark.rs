...
fn benchmark_infrastructure_deployment_iac() {
    ...
    assert!(has_iac || has_terraform || has_ansible,
            "Mask should cover Infrastructure as Code");

    let score = [has_iac, has_terraform, has_ansible, has_declarative, has_idempotent]
        .iter()
        .filter(|&&x| x)
        .count();
...
fn benchmark_infrastructure_deployment_containers() {
    ...
    assert!(has_containers || has_docker || has_kubernetes,
...
fn benchmark_infrastructure_deployment_cicd() {
    ...
    let score = [has_cicd, has_pipeline, has_github_actions, has_gitlab, has_automation]
        .iter()
        .filter(|&&x| x)
        .count();
...
fn benchmark_infrastructure_deployment_monitoring() {
    ...
    let score = [has_monitoring, has_prometheus, has_grafana, has_logging, has_alerting]
        .iter()
        .filter(|&&x| x)
        .count();

    assert!(score >= 2, "Mask should cover monitoring/observability (found {}/5)", score);
...
fn benchmark_infrastructure_deployment_ha() {
    ...
    let score = [has_ha, has_loadbalancing, has_failover, has_redundancy, has_clustering]
        .iter()
        .filter(|&&x| x)
        .count();

    assert!(score >= 2, "Mask should cover high availability (found {}/5)", score);
