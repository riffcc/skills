...
fn v2_before_after_diffs() {
    ...
    let has_diff_notation = content.contains("```diff") ||
                           content.contains("Before:") ||
                           content.contains("After:");

    let has_exact_text = content.contains("exact") ||
                        content.contains("specific") ||
                        content.contains("verbatim");

    // Must mention showing diffs in output
    let has_diff_in_output = content.contains("Output") &&
                            (content.contains("before") || content.contains("diff"));
    ...
    let score = [has_before_after, has_diff_notation, has_exact_text,
                 has_diff_in_output, has_diff_example]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must provide before/after diff capability (got {}/5)", score);
}
...
fn v2_score_prediction() {
    ...
    let has_percentage = content.contains("%") ||
                        content.contains("percent") ||
                        content.contains("score:");

    let has_impact_analysis = content.contains("impact") ||
                             content.contains("effect") ||
                             content.contains("result");

    // Must mention predicting in output
    let has_prediction_in_output = content.contains("Output") &&
                                   (content.contains("predict") || content.contains("expect"));

    let score = [has_prediction, has_score_mention, has_percentage,
                 has_impact_analysis, has_prediction_in_output]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must predict score changes (got {}/5)", score);
}
...
fn v2_implementation_difficulty() {
    ...
    let has_complexity = content.contains("complex") ||
                        content.contains("simple");

    let has_time_estimate = content.contains("time") ||
    ...
    let score = [has_difficulty, has_difficulty_levels, has_complexity,
                 has_time_estimate, has_difficulty_in_output]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must rate implementation difficulty (got {}/5)", score);
}
...
fn v2_priority_framework() {
    ...
    let has_ordering = content.contains("order") ||
    ...
    let score = [has_priority, has_impact_effort, has_high_priority,
                 has_ordering, has_priority_example]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must provide priority framework (got {}/5)", score);
}
...
fn v2_cross_domain_patterns() {
    ...
    let has_reusable = content.contains("reuse") ||
                      content.contains("apply") ||
                      content.contains("generalize");
    ...
    let score = [has_pattern_mention, has_cross_domain, has_reusable,
                 has_pattern_library, has_pattern_example]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must identify cross-domain patterns (got {}/5)", score);
}
...
fn v2_meta_learning_capability() {
    ...
    let has_learning = content.contains("learn") ||
                      content.contains("discover") ||
                      content.contains("extract");

    let has_feedback_loop = content.contains("feedback") ||
    ...
    let has_meta_example = (content.contains("### Example") || content.contains("**Example")) &&
                          (content.contains("meta") ||
                           content.contains("learn") ||
                           content.contains("itself"));

    let score = [has_meta, has_self_improvement, has_learning,
                 has_feedback_loop, has_meta_in_mission, has_meta_example]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    assert!(score >= 5, "V2: Must demonstrate meta-learning capability (got {}/6)", score);
}
