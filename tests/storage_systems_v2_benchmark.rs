...
fn v2_moosefs_deployment_detailed() {
    ...
    let score = [has_master, has_chunkserver, has_metalogger, has_client,
                 has_config_example, has_config_files, has_replication]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 6/7
    assert!(score >= 6, "V2: Must have detailed MooseFS deployment coverage (got {}/7)", score);
}
...
fn v2_storage_performance_benchmarking() {
    ...
    let has_throughput = content.contains("throughput") ||
                        content.contains("MB/s") ||
    ...
    let has_fio_commands = content.contains("```") &&
                          (content.contains("fio") || content.contains("benchmark"));

    let score = [has_fio, has_iops, has_throughput, has_latency,
                 has_benchmark_example, has_fio_commands]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 5/6
    assert!(score >= 5, "V2: Must have detailed performance benchmarking coverage (got {}/6)", score);
}
...
fn v2_failure_recovery_procedures() {
    ...
    let has_recovery_steps = content.contains("recovery") ||
                            content.contains("restore") ||
                            content.contains("repair");
    ...
    let score = [has_disk_failure, has_node_failure, has_master_failure,
                 has_recovery_steps, has_failure_example, has_recovery_commands]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 5/6
    assert!(score >= 5, "V2: Must cover failure recovery procedures (got {}/6)", score);
}
...
fn v2_capacity_planning() {
    ...
    let has_growth_projection = content.contains("growth") ||
                               content.contains("scale") ||
                               content.contains("expansion");

    let has_disk_space = content.contains("TB") ||
                        content.contains("PB") ||
                        content.contains("storage space");
    ...
    let score = [has_capacity_mention, has_growth_projection, has_disk_space,
                 has_replication_overhead, has_capacity_example]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must have capacity planning coverage (got {}/5)", score);
}
...
fn v2_monitoring_observability() {
    ...
    let has_alerts = content.contains("alert") ||
    ...
    let score = [has_prometheus, has_grafana, has_specific_metrics,
                 has_alerts, has_monitoring_example, has_exporter]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 5/6
    assert!(score >= 5, "V2: Must have detailed monitoring coverage (got {}/6)", score);
}
...
fn v2_production_ready_examples() {
    ...
    println!("V2 Hardware Specs: {}", if has_hardware_specs { "Present" } else { "Missing" });
    println!("V2 Network Topology: {}", if has_network { "Present" } else { "Missing" });
    println!("V2 Configurations: {}", if has_configs { "Present" } else { "Missing" });
    ...
    assert!(has_hardware_specs, "V2: Must include hardware specifications");
