...
fn v2_cap_theorem_tradeoffs() {
    ...
    let score = [has_cp_system, has_ap_system, has_cap_example, has_tradeoff_explanation]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    assert!(score >= 3, "V2: Must score 3/4 or higher on CAP depth (got {}/4)", score);
}
...
fn v2_consensus_failure_scenarios() {
    ...
    let score = [has_split_brain, has_quorum_loss, has_leader_failure,
                 has_network_partition, has_failure_example]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    assert!(score >= 4, "V2: Must cover 4/5 failure scenarios (got {}/5)", score);
}
...
fn v2_production_examples() {
    ...
    println!("V2 Configurations: {}", if has_yaml || has_toml || has_code { "Present" } else { "Missing" });
    println!("V2 Metrics: {}", if has_metrics { "Covered" } else { "Missing" });
    println!("V2 Thresholds: {}", if has_thresholds { "Specified" } else { "Missing" });
    ...
    assert!(has_yaml || has_toml || has_code, "V2: Must include actual configuration code blocks");

    // V2 requires specific metrics/thresholds
    assert!(has_metrics && has_thresholds, "V2: Must specify concrete metrics and thresholds");
...
fn v2_observability_depth() {
    ...
    let has_specific_exporters = has_node_exporter || has_postgres_exporter ||
    ...
    let has_alert_thresholds = content.contains(" > ") ||
                               content.contains(" < ") ||
    ...
    let score = [has_specific_exporters, has_specific_metrics, has_alerting,
                 has_alert_thresholds, has_dashboards, has_tracing_detail]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 5/6
    assert!(score >= 5, "V2: Must have deep observability coverage (got {}/6)", score);
}
...
fn v2_antipatterns() {
    ...
    let warns_premature_optimization = content.contains("premature") ||
                                      (content.contains("don't") && content.contains("consensus") && content.contains("simple"));
    ...
    let score = [has_avoid_section, has_dont_statements, has_antipattern_examples,
                 warns_premature_optimization, warns_network_assumptions]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    // V2 requires 4/5
    assert!(score >= 4, "V2: Must cover anti-patterns extensively (got {}/5)", score);
}
